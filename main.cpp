      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          for(int i = 0; i<sensor_fusion.size();i++){
            ...
            // check if any car is in current lane
            if((d<(2+4*lane+2)) && (d>(2+4*lane-2))){
             ...
             // check if action is needed, if too close, keep lane and follow the front car first
              if((check_car_s > car_s) && (check_car_s - car_s)<30){
                too_close = true;
                while(ref_v>check_speed){
                  ref_v -= 0.224;
                }
          		}
          	}
          ...
          if(too_close){
            lane = choose_next_lane(lane,ref_v, previous_size, sensor_fusion);
          }
          else if (ref_v< 49){
            ref_v += 0.224;
          }
          ...
          // part of code that control the driving
          vector<double> ptsx;
          vector<double> ptsy;
